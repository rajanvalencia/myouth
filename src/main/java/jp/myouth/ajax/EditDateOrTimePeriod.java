...
@WebServlet("/apis/ajax/forms/editDateOrTimePeriod")
public class EditDateOrTimePeriod extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if(questionType.equals("date")) {
        	res = db1.updateDatePeriod(questionId, start, end);
        } else if(questionType.equals("time")) {
        	res = db1.updateTimePeriod(questionId, start, end);
        }
        db1.close();
        
        Notifications db2 = new Notifications();
        db2.open();
        db2.insertEventNotification(event, userId, "が「"+title+"」の質問「"+question+"」の開始日を「"+start+"」から「"+end+"」に変更しました。");
        db2.close();
