...
@WebServlet("/apis/ajax/forms/addDateOrTimePeriod")
public class AddDateOrTimePeriod extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if(questionType.equals("date")) {
        	res = db1.insertDatePeriod(questionId, start, end);
        } else if(questionType.equals("time")) {
        	res = db1.insertTimePeriod(questionId, start, end);
        }
        db1.close();
        
        Notifications db2 = new Notifications();
        db2.open();
        db2.insertEventNotification(event, userId, "が「"+title+"」の質問「"+question+"」の形式を「"+questionType+"」に変更し、開始を「"+start+"」、修了を「"+end+"」に設定しました。");
        db2.close();
