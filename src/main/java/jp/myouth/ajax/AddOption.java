...
@WebServlet("/apis/ajax/forms/addOption")
public class AddOption extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        Boolean res = db1.insertOptions(questionId, optionId, option);
        db1.close();
        
        Notifications db2 = new Notifications();
        db2.open();
        db2.insertEventNotification(event, userId, "が「"+title+"」の質問「"+question+"」に"+option+"を追加しました。");
        db2.close();
